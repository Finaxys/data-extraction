...
public class YahooStockGateway implements RefDataGateway<Stock> {
	...
	public YahooStockGateway(CloseableHttpClient httpClient,
			ContentType contentType, Parser<Stock> parser) {
		super();
	...
	@Override
	public List<Stock> getRefData() throws Exception {
		try {
			List<String> params = new ArrayList<String>();
			byte[] data = YahooGatewayHelper.executeYQLQuery(STOCK_QUERY, params, contentType, httpClient, context);
			if (data.length > 0)
				return parser.parse(new Document(contentType, DataType.REF, DataClass.Stock, YahooGatewayHelper.Y_PROVIDER_SYMB, data));
			else
				return null;
		} catch (Exception e) {
			e.printStackTrace();
