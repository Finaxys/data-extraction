...
public class EbfXlsHistInterbankRateDataParser implements Parser<InterbankRateData> {
	...
	public List<InterbankRateData> parse(Document document) throws Exception {
		...
		while (cellIterator.hasNext()
				&& !(date = cellIterator.next().toString()).equals(""))
			dates.add(formatter.parseDateTime(date));

		String bucket = "";
		while (rowIterator.hasNext()
				&& !(bucket = (cellIterator = (row = rowIterator.next())
						.cellIterator()).next().toString()).equals("")) {

			Iterator<DateTime> datesIterator = dates.iterator();
			String value = "";
			InterbankRateData rate;
			while (cellIterator.hasNext()
					&& !(value = cellIterator.next().toString()).equals("")) {
				try {
