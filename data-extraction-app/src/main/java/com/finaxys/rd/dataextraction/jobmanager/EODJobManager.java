...
public class EODJobManager {
	...
	public void scheduleEODOptionQuotesJobs(char provider,
		...
		for (Exchange exchange : exchanges) {
			if (exchange.getProvider() == provider) {
				...
				scheduleJob(
						...
						jobData, exchange.getCloseTime(), exchange.getDelay());
			}
		}
	}

	public void scheduleEODOQuotesJobs(char provider,
			String exchangeDaoBeanName, String productDaoBeanName,
			String serviceBeanName, String publishingGatewayBeanName,
			Integer bucket) throws IOException, SchedulerException {
		...
		for (Exchange exchange : exchanges) {
			if (exchange.getProvider() == provider) {
				...
				Map<String, Object> jobData = getJobData(products,
						serviceBeanName, publishingGatewayBeanName, bucket);
				scheduleJob(com.finaxys.rd.dataextraction.job.EODDataJob.class,
						jobData, exchange.getCloseTime(), exchange.getDelay());
			}
		}
	}

	public void scheduleEODIndexQuotesJobs(char provider,
			String exchangeDaoBeanName, String productDaoBeanName,
			String serviceBeanName, String publishingGatewayBeanName,
			Integer bucket) throws IOException, SchedulerException {
	...
	public void scheduleEODStockQuotesJobs(char provider,
			String exchangeDaoBeanName, String productDaoBeanName,
			String serviceBeanName, String publishingGatewayBeanName,
			Integer bucket) throws IOException, SchedulerException {
	...
	public void scheduleEODFXRatesJobs(char provider,
			String exchangeDaoBeanName, String productDaoBeanName,
			String serviceBeanName, String publishingGatewayBeanName,
			Integer bucket) throws IOException, SchedulerException {
	...
	private Map<String, Object> getJobData(List<? extends MarketData> products,
		...
		jobData.put("service",
				(EODDataService<?, ?>) context.getBean(serviceBeanName));
	...
	private void scheduleJob(Class<?> jobClass, Map<String, Object> jobData,
			LocalTime closeTime, Integer delay) throws SchedulerException {
		...
		CronTrigger trigger = createTrigger(
				"trigger-" + jobClass.getSimpleName() + "-"
						+ jobData.hashCode(), jobDetail, closeTime, delay);
	...
	private CronTrigger createTrigger(String name, JobDetail jobDetail,
	...
	private String createCronExpression(LocalTime closeTime, Integer delay) {
		...
		return closeTime.getSecondOfMinute() + " "
				+ (closeTime.getMinuteOfHour() + 2) + " "
